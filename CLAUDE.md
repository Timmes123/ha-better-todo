...
- Repo: **github.com/Timmes123/ha-better-todo** (public, MIT). v0.3.0 is released, installed via HACS on the live HA, and verified working (backend logic tested over WS; todo/calendar entities confirmed). CI (hassfest + HACS action) green on main. v0.3 added: due times, reminders (events), advanced schedules (multi-weekday, last day, nth weekday, until/max), tags, mirror todo + calendar entities per list, card sort/dnd/editor/7 languages. All features are original clean-room work; do not name or reference third-party integrations anywhere in the repo (public files!) — user's explicit wish.
